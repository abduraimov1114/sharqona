...
<head>
	...
	<script src="assets/js/jquery-1.11.3.min.js"></script>
		...
		<?php if (isset($_GET['service_id'])) {
			$kelgan_service_id = $_GET['service_id'];
			$kelgan_service_id = mysqli_real_escape_string($connection, $kelgan_service_id);
		} 
		 $servicni_topish = "SELECT * FROM services WHERE service_id = '$kelgan_service_id'";
		 $service_tanlov = mysqli_query($connection, $servicni_topish);

		 if (!$service_tanlov) {
		 	die("Bunday IDli servicni qidirishda xatolik");
		 }
		 ...
		 if ($count == 1) {
		 	while ($row = mysqli_fetch_assoc($service_tanlov)) {
		 		$tanlangan_service_id = $row["service_id"];
		 		$tanlangan_service_name = $row['service_name'];
		 		$tanlangan_service_price = $row['service_price'];
		 		$tanlangan_service_product = $row['service_pro_id'];
		 		$service_street_price = $row['service_street_price'];
		 		$barcode = $row['service_barcode'];
		 	}
		 }
		 else {
		 	header("Location: /sharqona/services.php");
		 	$_SESSION['xabar'] = "xato";
		 }
		...
		?>
		<fieldset>
			<legend><strong>Xizmat turini o'zgartirish</strong></legend>
		<div class="row">
			<div class="col-md-12">
				<div class="panel-body">
					...
					<form role="form" method="post" class="form-horizontal form-groups-bordered validate" novalidate="novalidate" onkeypress="return event.keyCode != 13;">
						...
						<div class="form-group">
									...
									<select name="maxsulotlar" class="select2" data-allow-clear="true">
										<option value="">Sarflanmaydi</option>
											...
											while ($row = mysqli_fetch_assoc($proquery)) {
												...
												if ($maxsulot_id == $tanlangan_service_product) {
													$selected = "selected";
												}
												else {
													$selected = "";
											 ...
											 ?>
												<option <?php echo $selected; ?> value="<?php echo $maxsulot_id; ?>"><?php echo "$maxsulot_ismi"; ?></option>
											<?php } ?>
									</select>
								...
								<div class="col-sm-6">
									<div class="label label-danger">Narxi (T)</div>
										<input type="number" class="form-control input-lg" name="xiz_narxi" step="any" min="500" data-validate="number required" placeholder="Xizmat narxi (T)" value="<?php echo $tanlangan_service_price; ?>">
								</div>

								<div class="col-sm-6">
									<div class="label label-danger">Narxi (K)</div>
										<input type="number" class="form-control input-lg" name="street_price" step="500" min="500" data-validate="number required" placeholder="Xizmat narxi (K)" value="<?php echo $service_street_price; ?>">
								</div>
						...
						<div class="form-group">
							<button type="submit" name="edit_service" data-loading-text="Iltimos kuting..." class="btn btn-success">Saqlash</button>
		...
		<?php 
			...
			if (isset($_POST['edit_service'])) {
				...
				if (empty($_POST['xizmat']) OR empty($_POST['xiz_narxi']) OR $_POST['xiz_narxi'] < 0 OR empty($_POST['street_price']) OR $_POST['street_price'] < 0) {
					$_SESSION['xabar'] = "xato";
					header("Location: /sharqona/services.php?source=add_service");
				}
					...
					$product_choosed = mysqli_real_escape_string($connection, $product_choosed);
					$newbarcode = mysqli_real_escape_string($connection, $newbarcode);

					$service_tayorgarlik = "UPDATE services SET service_name = '$service_title', service_price = '$service_price', service_barcode = '$newbarcode', service_street_price = '$street_price', service_pro_id = '$product_choosed' WHERE service_id = '$tanlangan_service_id'";
