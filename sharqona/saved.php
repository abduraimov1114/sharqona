...
<div class="page-container">
    ...
    <table class="table table-bordered table-striped datatable" id="table-1">
        <h3 style="text-align: center;">Saqlangan ishlar</h3>
        <thead>
        <tr class="replace-inputs">
            <th width="10%" class="text-center" style="background: #00A651;color:white;">San'a</th>
            <th width="5%" class="text-center" style="background: #00A651;color:white;">Tabel</th>
            <th class="text-center" style="background: #00A651;color:white;">Tashkilot nomi</th>
            <th class="text-center" width="10%" style="background: #00A651;color:white;">Ma'lumot</th>
            <th class="text-center" style="background: #00A651;color:white;">Ism</th>
            <th class="text-center" style="background: #00A651;color:white;">Telefon</th>
            <?php if ($_SESSION['ses_user_role'] !== 'Direktor'): ?>
            <th class="text-center" width="18%" style="background: #00A651;color:white;">Muomala</th>
            <?php endif ?>
        </tr>
        </thead>
        <?
...
//        echo "<br/>".$saved_id[3]."<br/>";
        ...
        <tbody>
        <?
            $sorov="select * from saved INNER JOIN buyers ON saved.buyer_id=buyers.buyers_id WHERE saved.storage_id=2 ORDER BY saved.saved_id DESC";
            $func->queryMysql($sorov);
            $count1=$count;
            $soni = 0;
            while($row=$func->result->fetch_array(MYSQL_ASSOC)){
            ...
            <tr class="odd gradeX">
                ...
                <td class="text-center">
                    <?
                        if($row['buyer_id']!=10){
                        ?>
                            <a href="update.php?update=<? echo $row['saved_id']; ?>&orgid=<? echo $row['buyer_id']; ?>" class="btn btn-info btn-sm">O`zgartirish</a>
                        <?
                    }else{
                        ?>
                            <a href="update_street.php?update=<? echo $row['saved_id']; ?>" class="btn btn-info btn-sm">O`zgartirish</a>
                        <?}?>
                    <a href="dell_save.php?dell=<? echo $row['saved_id']; ?>&storage=2" class="btn btn-danger btn-sm" onClick="deleteqilish(this);return false;">O`chirish</a>
                </td>
                <?php endif ?>
            </tr>
            <?php $soni++ ?>
