...
<?php 
	if (isset($_GET['orgid'])) {
		...
		$count_organization = mysqli_num_rows($org_tanlov);
		if ($count_organization == 1) {
...
<form role="form" method="post"  onkeypress="return event.keyCode != 13;" action="analyzer.php" class="form-horizontal form-groups-bordered order">
  ...
  <div class="row hidden-print">
    <h2 class="text-center"><?php echo $tashkilot_ismi; ?></h2>
      ...
      ?>
      <?
      $sorov="select * from saved INNER JOIN buyers ON buyers.buyers_id=saved.buyer_id WHERE buyer_id=".$_GET['orgid'];
      ...
      <?
            ...
            ?>
    <div class="col-sm-6">
      <input id="user_name" value="<? echo $row['user_name']; ?>" class="form-control" type="text" name="user_name" placeholder="Ismi.."/>
    </div>
    <div class="col-sm-6">
    ...
    <tr class="pr-row row-item">
      ...
      <td>
        <select id="pr_name" name="product[__i__][name]" class="form-control input-md pr-name item-select">
          ...
          <?php if ($session_role == 'Direktor'): ?>
            <option value="383" data-price="100000" data-left="Not need">Matritsa dlya protsessor</option>
            <?php else: ?>
          <?php
      ...
      <td>
        <div class="input-group pr-quantity">
          <input id="pr_quantity" type="number" min="1" onkeyup="items()" name="product[__i__][quantity]" class="bold input-lg form-control pr-quantity item-quantity" value="1">
        </div>
