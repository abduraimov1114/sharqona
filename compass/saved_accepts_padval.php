...
<head>
    ...
    <title>Saqlangan ishlar Podval</title>
    <!-- Bu yerda scriptlar -->
...
<div class="page-container">
    ...
    <table class="table prices" id="table-1">
        <h3 style="text-align: center;">Saqlangan ishlar (Podval)</h3>
        <thead>
...
//        echo "<br/>".$saved_id[3]."<br/>";
        ...
        <tbody>
        <?
            $sorov="select * from saved INNER JOIN buyers ON saved.buyer_id=buyers.buyers_id WHERE saved.storage_id=1 ORDER BY saved.saved_id DESC";
            $func->queryMysql($sorov);
            $count1=$count;
            $soni = 0;
            while($row=$func->result->fetch_array(MYSQL_ASSOC)){
        ...
        ?>
            <tr class="odd gradeX">
                ...
                ?>
                    <td class="text-center"><? echo $row['buyer_id']."-".($row['saved_times']-1); ?></td>
                <?}else{?>
                ...
                <td class="text-center">
                    <?
                        if($row['buyer_id']!=10){
                        ?>
                            <a href="update_accept.php?update=<? echo $row['saved_id']; ?>&storage=1" class="btn btn-info"><i class="entypo-info"></i></a>
                        <?
                    }
                    else{
                        ?>
                            <a href="update_accept_street.php?update=<? echo $row['saved_id']; ?>&storage=1" class="btn btn-info"><i class="entypo-info"></i></a>
                        <?}?>
                    <a href="dell_save.php?dell=<? echo $row['saved_id']; ?>&storage=1" class="btn btn-danger" onClick="deleteqilish(this);return false;"><i class="entypo-trash"></i></a>
                </td>
            </tr>
            <?php $soni++ ?>
        <?
